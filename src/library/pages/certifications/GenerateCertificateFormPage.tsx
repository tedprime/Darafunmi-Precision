...
const GenerateCertificateFormPage: React.FC = () => {
  ...
  // Detect edit mode from ?id= param
  ...
  // Form fields — certNo is auto-generated by backend (read-only in edit mode only)
  const [certNo, setCertNo] = useState("");   // populated on load in edit mode; never sent on create
  ...
  // Load clients for dropdown — keep full details for auto-fill
  useEffect(() => {
    getClients({ limit: 200 }).then(({ data }: { data: { id: number; name: string; email?: string; phone?: string; address?: string; location?: string }[] }) =>
      setClients(data)
    ).catch(() => {});
  }, []);

  // Track whether the user manually changed the client dropdown (vs. edit-mode load)
  const [userPickedClient, setUserPickedClient] = useState(false);

  // Auto-fill customer fields only when the admin actively picks a client
  useEffect(() => {
  ...
  const buildPayload = () => {
    ...
    // Only include clientId if it's a valid non-empty number — omitting it
    // entirely avoids the server 500 caused by sending null for an integer field
    const clientIdNum = clientId.trim() ? Number(clientId) : undefined;

    return {
      // certificateNumber is intentionally omitted — backend auto-generates it
  ...
  const openEmailModal = () => {
    // Pre-fill with the client's email if available
    const client = clients.find((c) => String(c.id) === clientId);
  ...
  const pageSubtitle = isEditMode
    ? `Editing certificate ${certNo || `#${editId}`}`
    : "Select a client and service type — the certificate number will be generated automatically.";
          ...
          {activeTab === "form" && (
            <>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <Card>
                  <h3 className="text-lg font-semibold text-gray-800 mb-1">Certificate Information</h3>
                  {!isEditMode && (
                    <p className="text-xs text-gray-400 mb-4">
                      The certificate number is auto-generated from the client and service type
                      (e.g. <span className="font-mono">DPT/ABC/CAL/2026/001</span>).
                    </p>
                  )}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Client — required, generates the cert number prefix */}
                    <div>
                      <label htmlFor="clientId" className="block text-sm font-medium text-gray-700 mb-1">Client<Req /></label>
                      <select
                        id="clientId"
                        value={clientId}
                        onChange={(e) => { setClientId(e.target.value); setUserPickedClient(true); }}
                    ...
                    {/* Service Type — required, determines CAL / MNT code in cert number */}
                    <div>
                      <label htmlFor="serviceType" className="block text-sm font-medium text-gray-700 mb-1">Service Type<Req /></label>
                      <select
                        id="serviceType"
                        value={serviceType}
                        onChange={(e) => setServiceType(e.target.value as "calibration" | "maintenance" | "")}
                        className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm bg-white"
                      >
                    ...
                    {/* Cert number — read-only in edit mode, hidden on create */}
                    {isEditMode && certNo && (
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Certificate Number</label>
                        <div className="mt-1 px-3 py-2 border border-gray-200 rounded-md bg-gray-50 text-sm font-mono text-gray-700 select-all">
                          {certNo}
                        </div>
                      </div>
                    )}
                ...
                <Card>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input id="custName" type="text" placeholder="Select a client above to auto-fill" value={custName} onChange={(e) => setCustName(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                    </div>
                    <div>
                      <label htmlFor="custAddress" className="block text-sm font-medium text-gray-700 mb-1">Customer Address<Req /></label>
                      <input id="custAddress" type="text" placeholder="Select a client above to auto-fill" value={custAddress} onChange={(e) => setCustAddress(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                    </div>
                  </div>
                </Card>
                ...
                <Card>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <label htmlFor="expDate" className="block text-sm font-medium text-gray-700 mb-1">Expiry Date<Req /></label>
                      <input id="expDate" type="date" value={expDate} onChange={(e) => setExpDate(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                    </div>
                  </div>
                </Card>
              ...
              {!hasPreviewData ? (
                ...
                <>
                  ...
                  <div className="flex justify-center mt-6 space-x-4">
                    {/* "Back to Form" becomes "Edit Certificate" when viewing an existing cert */}
                    ...
                    {savedId && (
                      <>
                        ...
                        <Button variant="secondary" className="flex items-center" onClick={openEmailModal}>
                          <Send size={16} className="mr-2" /> Send to Customer
                        </Button>
                      </>
                    )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h2 className="text-base font-semibold text-gray-800">Send Certificate to Customer</h2>
              <button onClick={() => setEmailModalOpen(false)} className="text-gray-400 hover:text-gray-600"><X size={18} /></button>
            </div>
            <div className="p-6">
              {certNo && (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-100 rounded-md text-sm">
                  <span className="text-gray-500">Certificate: </span>
                  <span className="font-mono font-medium text-blue-700">{certNo}</span>
                </div>
              )}
              <p className="text-sm text-gray-600 mb-4">
                The certificate PDF will be generated (if needed) and emailed to the address below.
              </p>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 mb-5"
              />
              <div className="flex gap-3">
                <button
                  onClick={handleSendEmail}
                  disabled={sending}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700 disabled:opacity-50"
                >
                  <Send size={14} />
