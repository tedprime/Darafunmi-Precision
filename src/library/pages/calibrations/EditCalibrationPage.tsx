...
import { getCalibration, updateCalibration } from "../../../services/calibration.jsx";
...
const EditCalibrationPage: React.FC = () => {
  ...
  const [performedByTitle, setPerformedByTitle] = useState("");
  ...
  const [wmvCertNo,  setWmvCertNo]  = useState("");
  ...
  /* Load clients + existing record */
  useEffect(() => {
    if (!id) return;
    Promise.all([
      getClients({ limit: 200 }).catch(() => ({ data: [] })),
      getCalibration(id),
    ]).then(([clientsRes, cal]: [any, any]) => {
      setClients(clientsRes.data ?? []);

      // Pre-fill all fields
      setEquipmentName(str(cal.equipmentName));
      setSerialNumber(str(cal.serialNumber));
      setCalibrationDate(str(cal.calibrationDate).split("T")[0]);
      setNextDueDate(cal.nextDueDate ? str(cal.nextDueDate).split("T")[0] : "");
      setRecalibDate(cal.recommendedRecalibDate ? str(cal.recommendedRecalibDate).split("T")[0] : "");
      setTechnician(str(cal.technician));
      setPerformedByTitle(str(cal.performedByTitle));
      setLocation(str(cal.location));
      setServiceType((cal.serviceType as "calibration" | "maintenance") || "calibration");
      if (cal.clientId) setClientId(String(cal.clientId));

      setCustomerName(str(cal.customerName));
      setCustomerCompanyName(str(cal.customerCompanyName));
      setCustomerEmail(str(cal.customerEmail));
      setCustomerPhone(str(cal.customerPhone));
      setCustomerAddress(str(cal.customerAddress));

      setScale(str(cal.scale));
      setScaleRange(str(cal.scaleRange));
      setScaleDivision(str(cal.scaleDivision));
      setMaxScaleError(str(cal.maxScaleError));
      setRefInst(str(cal.referenceInstrument));
      setRefInstSN(str(cal.referenceInstrumentSN));

      setTemperature(str(cal.temperature));
      setHumidity(str(cal.humidity));

      setPhysExam(str(cal.physicalExamText));
      setTableType((cal.tableType as "3col" | "4col") || "3col");
      setTableUnit(str(cal.tableUnit) || "cm");
      setSonLabRef(str(cal.sonLabRef));
      setWmvCertNo(str(cal.wmvCertNo));
      setNotes(str(cal.notes));

      if (cal.measurements?.length) {
        setMeasurements(cal.measurements.map((m: any) => ({
          standardValue: str(m.standardValue),
          measuredValue: str(m.measuredValue),
          asFoundValue:  str(m.asFoundValue),
          asLeftValue:   str(m.asLeftValue),
          deviation:     str(m.deviation),
        })));
      }
    }).catch((err: any) => {
      setLoadError(err?.message || "Failed to load calibration record.");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.message || "Failed to update calibration record. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return (
      <Layout pageTitle="Edit Calibration Record" pageSubtitle="Loading…">
  ...
  if (loadError) {
    return (
      <Layout pageTitle="Edit Calibration Record" pageSubtitle="">
        <div className="flex flex-col items-center justify-center py-24 text-center">
          <AlertCircle size={28} className="text-red-400 mb-3" />
          <p className="text-gray-700 font-medium">{loadError}</p>
          <button onClick={() => navigate("/calibrations/history")} className="mt-4 px-4 py-2 text-sm bg-gray-100 rounded-lg hover:bg-gray-200">
            Back to History
          </button>
        </div>
      </Layout>
  ...
  return (
    <Layout
      pageTitle="Edit Calibration Record"
      pageSubtitle="Update calibration details. Changes are saved immediately."
    >
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            {/* Customer */}
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
              <SectionHead icon={User} title="Customer Information" sub="Appears on the certificate." />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1.5">Customer Name <Req /></label>
                  <input value={customerName} onChange={(e) => setCustomerName(e.target.value)} className={inputCls} placeholder="e.g. John Doe" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1.5">Customer Address <Req /></label>
                  <input value={customerAddress} onChange={(e) => setCustomerAddress(e.target.value)} className={inputCls} placeholder="e.g. 12 Industrial Road, Lagos" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1.5">Company / Organisation Name <Req /></label>
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1.5">Customer Email <Req /></label>
                  <input
                    type="email"
                    value={customerEmail}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1.5">Customer Phone <Req /></label>
            ...
            <div className="bg-blue-50 rounded-xl border border-blue-100 p-4">
              <div className="flex items-start gap-2.5">
                ...
                <p className="text-xs text-blue-700 leading-relaxed">
                  Updating this record will recompute the calibration status from the Next Due Date.
                  If a certificate has already been generated, you'll need to regenerate it to reflect changes.
                </p>
              </div>
            </div>
...
export default EditCalibrationPage;
